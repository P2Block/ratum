...
fn build_header_v2_inverts_from_header_and_share_extranonce() {
    ...
    assert_eq!(build_header_v2(&job, &s, &h.merkle_root, None).unwrap(), h);
}

#[test]
fn rebuilds_a_correct_share() {
    let (mut v, s) = setup();
    let rebuilt = v.rebuild_checked(&s, NOW).unwrap();
...
fn the_header_is_always_the_sia_profile() {
    let (mut v, s) = setup();
    let h = built_header(&v.rebuild_checked(&s, NOW).unwrap());
...
fn the_time_offset_flag_decides_whether_the_offset_moves_the_block_time() {
    ...
    let b = s.blake2b;
    let h = built_header(&v.rebuild_checked_ignoring_target(&s, NOW).unwrap());
...
fn the_header_counts_the_coinbase_among_its_transactions() {
    ...
    assert_eq!(
        built_header(&v.rebuild_checked_ignoring_target(&s, NOW).unwrap()).txcount,
        1,
        "the coinbase alone"
    );
...
fn a_subsidy_only_header_counts_only_the_coinbase() {
    ...
    let rebuilt = v
        .rebuild_checked_ignoring_target(&s, NOW)
...
fn a_quickdiff_share_is_rebuilt_from_its_target_byte() {
    let (mut v, s) = setup();
    let plain = v.rebuild_checked(&s, NOW).unwrap();
    let mut quick = s.clone();
    quick.quickdiff = true;
    let with_quickdiff = v.rebuild_checked(&quick, NOW).unwrap();
...
fn a_target_byte_that_is_not_a_difficulty_exponent_is_refused() {
    ...
    for byte in [0xffu8, 0x80, 64] {
        let mut s = base.clone();
        s.target_byte = byte;
        assert_eq!(v.rebuild_checked(&s, NOW), Err(RejectReason::BadTarget), "byte {byte:#04x}");
    }
}
...
fn rejects_a_coinbase_without_the_pool_tag() {
    ...
    assert_eq!(v.rebuild_checked(&share, NOW), Err(RejectReason::MissingPoolTag));
}
...
fn located(p: &SharePolicy, tagging: &ScriptSigTags) -> (usize, String) {
    ...
    let parsed = bitcoin::transaction::parse_coinbase(&coinbase_tx).expect("a parseable coinbase");
...
fn a_coinbase_with_only_the_pool_tag_records_an_empty_secondary_tag() {
    let (mut v, s) = setup();
    assert_eq!(v.rebuild_checked(&s, NOW).unwrap().tag_secondary, "");
}
...
fn accepts_the_version_3_eleven_byte_prime_push() {
    ...
    let mut v = Verifier::new(wide.clone(), Arc::new(Mutex::new(AcceptedShareHashes::default())));
    record(&mut v, &split(), &[], NOW);
    let mut share = share_on(job_section(target_byte_index), cb.clone());
    share.target_byte = 5;
    let rebuilt = v
        .rebuild_checked_ignoring_target(&share, NOW)
    ...
    assert_eq!(v.rebuild_checked_ignoring_target(&share, NOW), Err(RejectReason::MissingPoolTag));
    let mut other = verifier();
    record(&mut other, &split(), &[], NOW);
    share.coinbase = Some(cb);
    assert_eq!(
        other.rebuild_checked_ignoring_target(&share, NOW),
...
fn rejects_a_target_byte_index_pointing_elsewhere() {
    ...
    s.job = Some(job);
    assert_eq!(v.rebuild_checked(&s, NOW), Err(RejectReason::TargetMismatch));
}
...
fn rebuilds_a_subsidy_only_share() {
    ...
    let rebuilt = v.rebuild_checked_ignoring_target(&share, NOW).unwrap();
...
fn rejects_a_coinbase_that_is_not_a_transaction() {
    ...
    assert_eq!(v.rebuild_checked(&s, NOW), Err(RejectReason::BadCoinbase));
}
...
fn find_a_difficulty_1_nonce() {
    for (name, (mut v, mut s)) in [("DIFF1_NONCE", setup()), ("DIFF1_NONCE_HARD", setup_hard())] {
        ...
        for offset in 0..16u32 {
            ...
            let rebuilt = v.rebuild_checked(&s, NOW + u64::from(offset)).expect("rebuild");
            let h = built_header(&rebuilt);
